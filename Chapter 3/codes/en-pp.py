...
def prep_heads(verb_index, sentence): 
	...
	for tok in sentence:
		if tok[3] == 'ADP':
			if tok[7] == 'case':
				if int(tok[0]) > int(verb_index): #the PP appears after the verb
			...
			if len(verbs) != 0:
				for verb in verbs:
					...
					if preps is not None:
						...
						#### VERB_lemma, VERB_lexical, Prep_lemma, Prep_lexical, PP_head_lemma, PP_head_lexical, PP_head_POS, VERB_id, Head_id #####
...
#					distance_pp2.append(str(tok[10]))
			...
			#	with io.open(args.output + directory[3 : ] + '-en-original.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in original:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-variant.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in variant:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-pp1.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in pp1:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-pp2.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in pp2:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-base1.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in base1:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-base2.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in base2:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-tuples1.txt', 'w', encoding = 'utf-8') as f:
			...
			#	with io.open(args.output + directory[3 : ] + '-en-tuples2.txt', 'w', encoding = 'utf-8') as f:
			#		f.write('\t'.join(w for w in ['VERB_lemma', 'VERB_lexical', 'Prep_lemma', 'Prep_lexical', 'PP_head_lemma', 'PP_head_lexical', 'PP_head_POS', 'VERB_id', 'Head_id']) + '\n')
			#		for tok in tuples2:
			#			f.write('\t'.join(w for w in tok) + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-pp1-lemma.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in pp1_lemmas:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-pp2-lemma.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in pp2_lemmas:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-pro1.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in pp1_pro:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-pro2.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in pp2_pro:
			#			f.write(tok + '\n')

			#	with io.open(args.output + directory[3 : ] + '-en-verb.txt', 'w', encoding = 'utf-8') as f:
			#		for tok in verb:
			#			f.write(tok + '\n')

				with io.open(args.output + directory[3 : ] + '-en-full.txt', 'w', encoding = 'utf-8') as f:
...
'''
				...
				with io.open('/Users/Silverlining/Desktop/PP-new/' + directory[3 : ] + '-en-statistics.txt', 'w', encoding = 'utf-8') as f:
